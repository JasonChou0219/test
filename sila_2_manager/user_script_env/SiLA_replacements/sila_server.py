...
class SiLA2Server:
    ...
    def __init__(self,
        ...
        # Todo: Implement code that checks if the used open port is used otherwise to avoid
        # multiple use of ports.

        if self.server_port is not None:
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            try:
                s.bind(('', self.server_port))  ## Try to open port
            except OSError as err:
                if err.errno is 98:  ## Errorno 98 means address already bound
                    logging.error(f"Specified port {self.server_port} is already occupied - ({err})")
        ...
        if self.server_port is None:
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.server_port = 55001  # range(55001, ....)
            while True:
                try:
                    s.bind(('', self.server_port))  ## Try to open port
                except OSError as err:
                ...
                s.close()
                logging.info(
                    f"No port specified or specified port is occupied. Registering Service on next free port: {self.server_port}")
                break
    ...
    def run(self, block: bool = True) -> None:
        ...
        self.service_detection.register_service(
            ...
            description={'version': __version__, 'server_name': self.SiLAService_feature.server_name, 'description': self.server_description},
    ...
    def stop_grpc_server(self, force: bool = False) -> None:
        ...
        if not force:
            ...
            pass
        self.grpc_server.stop(force)
        self.service_detection.unregister_service()
