...
class Pump:
    ...
    def enable_channel_addressing(self):
        ...
        if not is_channel_addressing:
            logging.info('Channel addressing was not enabled. Setting channel addressing to true.')
            get(route, params={'service_uuid': self.pump['uuid'], 'feature_identifier': 'DeviceService',
    ...
    def adjust_pump_settings(self):
        self.enable_channel_addressing()
        is_event_messages = get(route, params={'service_uuid': self.pump['uuid'], 'feature_identifier': 'DeviceService',
                                               'function_identifier': 'GetEventMessages', 'is_property': False,
                                               'parameters': [ADDRESS],
        ...
        if is_event_messages:
            ...
            event_messages_set = get(route, params={'service_uuid': self.pump['uuid'],
                                                    'feature_identifier': 'DeviceService',
                                                    'function_identifier': 'SetEventMessages', 'is_property': False,
                                                    'parameters': [ADDRESS, False],
                                                    'response_identifiers': 'EventMessagesSet'}).json()['response'][
                'EventMessagesSet']
            print(f'Event messages are enabled. Disabled event messages: {event_messages_set}')
            is_event_messages = get(route, params={'service_uuid': self.pump['uuid'],
                                                   'feature_identifier': 'DeviceService',
                                                   'function_identifier': 'GetEventMessages', 'is_property': False,
                                                   'parameters': [ADDRESS],
                                                   'response_identifiers': 'EventMessages'}).json()['response'][
                'EventMessages']
    ...
    def pump_set_volume_at_flow_rate(self, flow_rate=None, channels=[1, 2], volume=[80, 80],
        ...
        for i, channel in enumerate(channels):
            flow_rate_at_mode = get(route, params={'service_uuid': self.pump['uuid'],
                                                   ...
                                                   'parameters': [ADDRESS],
                                                   'response_identifiers': 'CurrentFlowRate'}).json()['response'][
                'CurrentFlowRate']
        ...
        for i, channel in enumerate(channels):
            ...
            current_pump_mode = get(route,
                                    params={'service_uuid': self.pump['uuid'],
                                            ...
                                            'parameters': [channel],
                                            'response_identifiers': 'CurrentPumpMode'}).json()['response'][
                'CurrentPumpMode']
            ...
            get(route, params={'service_uuid': self.pump['uuid'], 'feature_identifier': 'ParameterController',
                               ...
                               'parameters': [channel, volume[i]]})  # {'Channel':,'Volume': }
            ...
            get(route, params={'service_uuid': self.pump['uuid'], 'feature_identifier': 'ParameterController',
                               ...
                               'parameters': [channel, flow_rate[i]]})  # {'Channel': 'SetFlowRate': }
    ...
    def pump_time_at_flow_rate(self, flow_rate=None, channels=[1, 2], runtime=['10.5', '5.3'],
        ...
        for i, channel in enumerate(channels):
            ...
            if is_at_flow_rate_mode is False:
                get(route, params={'service_uuid': self.pump['uuid'], 'feature_identifier': 'ParameterController',
                                   'function_identifier': 'SetFlowRateMode', 'is_property': False,
                                   'parameters': [channel, True]})  # {'Channel':  'ChannelAddressing':}
        ...
        for i, channel in enumerate(channels):
            ...
            get(route, params={'service_uuid': self.pump['uuid'], 'feature_identifier': 'ParameterController',
                               ...
                               'parameters': [channel, runtime[i]]})  # {'Channel':  'RunTime':}
            # The flow rate is set according to the setting in the head of the function (in mL/min). Must be LESS or
            # EQUAL to the max. flow rate of the channel
            get(route, params={'service_uuid': self.pump['uuid'], 'feature_identifier': 'ParameterController',
                               'function_identifier': 'SetFlowRate', 'is_property': False,
    ...
    def pump_status(self):
        ...
        return_val = get(route, params={'service_uuid': self.pump['uuid'], 'feature_identifier': 'DeviceService',
                                        ...
                                        'parameters': [ADDRESS],
                                        'response_identifiers': 'CurrentPumpStatus'}).json()['response'][
            'CurrentPumpStatus']
        return return_val
