...
class DataTypeDefinition(DataType):
    ...
    def __init__(self, xml_tree_element):
        ...
        if hasattr(xml_tree_element.DataType, 'Basic'):
            self.sub_type = BasicType(
                xml_tree_element=xml_tree_element.DataType.Basic)
            self.python_type = self.sub_type.python_type
            self.sila_type = self.sub_type
        elif hasattr(xml_tree_element.DataType, 'List'):
            self.sub_type = ListType(
                xml_tree_element=xml_tree_element.DataType.List)
            self.python_type = self.sub_type.python_type
        elif hasattr(xml_tree_element.DataType, 'Structure'):
            self.sub_type = StructureType(
                xml_tree_element=xml_tree_element.DataType.Structure)
            self.sila_type = self.sub_type
        elif hasattr(xml_tree_element.DataType, 'Constrained'):
            self.sub_type = ConstrainedType(
                xml_tree_element=xml_tree_element.DataType.Constrained)
            self.python_type = self.sub_type.python_type
            self.sila_type = self.sub_type.sila_type
        elif hasattr(xml_tree_element.DataType, 'DataTypeIdentifier'):
            self.sub_type = DataTypeIdentifier(
                xml_tree_element=xml_tree_element.DataType.DataTypeIdentifier)
            self.sila_type = self.sub_type
