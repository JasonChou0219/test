...
class Command:
    ...
    def __init__(self, xml_tree_element, data_type_definitions=None):
        """
        Class initialiser
        ...
        if hasattr(xml_tree_element, 'Parameter'):
            for parameter in xml_tree_element.Parameter:
                ...
                if isinstance(obj.sub_type, DataTypeIdentifier):
                    # hacky to get to the real python data type
                    rel_type = data_type_definitions[obj.sub_type.sub_type]
                    obj.sub_type.python_type = rel_type.python_type
                    if obj.sub_type.is_basic:
                        obj.sub_type.default_value = BasicType.basic_data_types[
                            rel_type.sila_type]['default_value']
                    elif obj.sub_type.is_structure:
                        obj.sub_type.default_value = None
            ...
            obj = ParameterDataType(
                xml_tree_element=self.genParmeterVoidXMLTree())
            self.parameters[obj.identifier] = obj
        ...
        if hasattr(xml_tree_element, 'Response'):
            ...
            obj = ResponseDataType(
                xml_tree_element=self.genResponseVoidXMLTree())
            self.responses[obj.identifier] = obj
    ...
    def genParmeterVoidXMLTree(self):
        ...
        parameter_void = E.Parameter(E.Identifier('EmptyParameter'),
                                     ...
                                     E.Description(
            'An empty parameter data type used if no parameter is required.'),
            E.DataType(
            E.Basic('Void'),
        )
        )
    ...
    def genResponseVoidXMLTree(self):
        ...
        response_void = E.Parameter(E.Identifier('EmptyResponse'),
                                    ...
                                    E.Description(
            'An empty response data type used if no response is required.'),
            E.DataType(
            E.Basic('Void'),
        )
        )
